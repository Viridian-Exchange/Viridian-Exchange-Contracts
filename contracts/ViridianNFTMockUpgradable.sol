...
// abstract contract NonblockingReceiver is OwnableUpgradeable, ILayerZeroReceiver {
...
contract ViridianNFTMockUpgradable is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable, BaseRelayRecipient {
    ...
    mapping(uint256 => uint256) private openLimit;
     ...
     */
    function viewHashedTokenIds(uint256 _maxIndex) external view onlyOwner returns (uint256[] memory) {
        uint256[] memory _hashedTokenIdsList = new uint256[](_maxIndex);

        for (uint256 i = 0; i <= _maxIndex; i++) {
            _hashedTokenIdsList[i] = hashedTokenIds[i];
        }

        return _hashedTokenIdsList;
    }

    /**
     * @dev Owner can view the hashed token Ids.
     */
    ...
    function getOwnedNFTs() public view virtual returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i <= balanceOf(_msgSender()); i++) {
            _tokens[i] = tokenOfOwnerByIndex(_msgSender(), i);
        }
    ...
    /**
     * @dev Users can mint during the drop using the blockchain's native currency (ex: Ether on Ethereum).
     */
    function mint(
        uint256 _numMint,
        address _to
    ...
    /**
     * @dev Users can mint during the drop using the specified ERC20 token (WETH for genesis).
     */
    function ERC20Mint(
        uint256 _numMint,
        address _to
    ...
    /**
     * @dev Users can mint with USD on crossmint during the polygon drop on Ethereum using Ether.
     */
     ...
     */
    function allowOpening(uint256 _openLimit) public onlyOwner() {
        openingUnlocked[dropId.current()] = true;
        openLimit[dropId.current()] = _openLimit;
    }
    ...
    function open(uint256 _tokenId) public {
        ...
        require(_tokenId <= openLimit[dropId.current()], "Pack is not openable yet");
