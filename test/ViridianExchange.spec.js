...
const ViridianToken = artifacts.require('ERC20TokenGasless');
...
contract('ViridianExchange', (accounts) => {
  ...
  beforeEach(async () => {
    token = await ViridianToken.new(forwarderAddress);
    nft = await ViridianNFT.new(forwarderAddress);
    pack = await ViridianPack.new(nft.address, forwarderAddress);
    exchange = await ViridianExchange.new(token.address, nft.address, pack.address, forwarderAddress, treasuryAddress);
    exof = await ViridianExchangeOffers.new(token.address, nft.address, pack.address, forwarderAddress, treasuryAddress);
  ...
  it('transaction: nft should be able to be purchased with ERC20', async () => {
    ...
    const balanceBefore = await token.balanceOf.call(accounts[ 0 ])
    const balanceSellerBefore = await token.balanceOf.call(accounts[ 1 ])
    ...
    const balanceAfter = await token.balanceOf.call(accounts[ 0 ])
    const balanceSellerAfter = await token.balanceOf.call(accounts[ 1 ])
    ...
    assert.strictEqual(balanceSellerBefore.toString(), "0");
    assert.strictEqual(balanceSellerAfter.toString(), "475");
    expect(await ownedNFTs.length).to.equal(1);
  ...
  it('items: offer should be able to be created for existing NFT', async () => {
    ...
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await exof.makeOffer(accounts[1], ['2', '3'], [], '300', ['1'], [], '100', token.address, "1", false, "0");
  ...
  it('items: offer should not be able to be created for existing NFT when not owned NFTs are included', async () => {
    ...
    await truffleAssert.reverts(
      exof.makeOffer(accounts[1], ['1', '3'], [], '300', ['1'], [], '100', token.address, "1", false, "0"),
  ...
  it('items: offer should not be able to be created for existing NFT when not owned NFTs are included', async () => {
    ...
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await truffleAssert.reverts(
      exof.makeOffer(accounts[1], ['2', '3'], [], '300', ['3'], [], '100', token.address, "1", false, "0"),
...
//   })
  ...
  it('transaction: should be able to accept ERC20 offer', async () => {
    ...
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await exof.makeOffer(accounts[1], [], [], '100', [], [], '200', token.address, "1", false, "0")
    let offers = await exof.getOffers.call({from: accounts[1]});
    let userOffers = await exof.getOffersFromUser(accounts[1]);
    console.log("UL: " + offers[0]);
    expect(await offers.length).to.equal(1);
  ...
  it('transaction: should be able to accept just NFT offer', async () => {
    ...
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await nft.mint(accounts[0], "https://viridian-nft-metadata.s3.us-east-2.amazonaws.com/vmd3.json");
    await exof.makeOffer(accounts[1], ['2', '3'], [], '0', ['1'], [], '0', token.address, "1", false, "0")
    let offers = await exof.getOffers.call({from: accounts[1]});
    let userOffers = await exof.getOffersFromUser(accounts[1]);
    console.log("UL: " + offers[0]);
    expect(await offers.length).to.equal(1);
...
//   })  
  ...
  it('transaction: should be able to accept ERC20/NFT hybrid offer', async () => {
    ...
    assert.strictEqual(balanceAfter.toString(),  "199999999999999999999999295");
    assert.strictEqual(balanceBefore1.toString(), "500");
    assert.strictEqual(balanceAfter1.toString(), "685");
  ...
  it('transaction: should not be able to accept ERC20/NFT hybrid offer when balance is too low', async () => {
   //await token.transfer(accounts[1], '500');
    ...
    await token.approve(exof.address, 100, {from: accounts[1]});
    //await token.approve(exchange.address, 300);
    await exof.makeOffer(accounts[2], ['2', '3'], [], '300', ['1'], [], '100', token.address, "1", false, "0", {from: accounts[1]})
